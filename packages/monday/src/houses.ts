...
export const createOrUpdateHouse = mutation({
  args: {
  ...
  handler: async (ctx, args) => {
    ...
    if (houseId) {
      ...
      imageForDeletion = house.imageId ?? null;

      await ctx.db.patch(houseId, {
        name: args.name,
      ...
      // Create Flow
      houseId = await ctx.db.insert("houses", {
        name: args.name,
        description: args.description,
        address: args.address,
        imageId: args.imageId,
    ...
    return await ctx.db.get(houseId);
  },
});
